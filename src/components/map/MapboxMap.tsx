...
export function MapboxMap({ 
  ...
  const initializeMap = () => {
    ...
    // Validar coordenadas do centro antes de inicializar
    const validCenterLat = isValidCoordinate(centerLat, centerLng) ? centerLat : -19.9167
    const validCenterLng = isValidCoordinate(centerLat, centerLng) ? centerLng : -43.9345
    ...
    console.log('🗺️ [MAPBOX] Inicializando mapa com coordenadas:', { validCenterLat, validCenterLng })

    map.current = new window.mapboxgl.Map({
      container: mapContainer.current,
      style: 'mapbox://styles/mapbox/streets-v12',
      center: [validCenterLng, validCenterLat],
      zoom: zoom,
  ...
  useEffect(() => {
    if (mapLoaded && map.current && isValidCoordinate(centerLat, centerLng)) {
  ...
  const updateMarkers = () => {
    ...
    const validOrders = orders.filter(order => {
      const isValid = isValidCoordinate(order.latitude, order.longitude)
      if (!isValid) {
        console.warn('⚠️ [MAPBOX] Pedido com coordenadas inválidas ignorado:', {
          id: order.id,
    ...
    console.log(`📊 [MAPBOX] Pedidos válidos: ${validOrders.length}/${orders.length}`)

    validOrders.forEach((order, index) => {
      console.log(`📌 [MAPBOX] Criando marcador ${index + 1}:`, {
        id: order.id,
        customerName: order.customerName,
      ...
      try {
        ...
        // Criar marcador no mapa com coordenadas validadas
        ...
        console.log(`✅ [MAPBOX] Marcador ${index + 1} adicionado com sucesso`)

      } catch (error) {
        console.error(`❌ [MAPBOX] Erro ao criar marcador ${index + 1}:`, error, {
          order: order,
          coordinates: [order.longitude, order.latitude]
        })
      }
    ...
    console.log(`🎯 [MAPBOX] Total de marcadores criados: ${markersRef.current.length}`)
  }

  const showOrderPopup = (order: any) => {
    if (!map.current || !window.mapboxgl || !isValidCoordinate(order.latitude, order.longitude)) return
    ...
    try {
        ...
        .setHTML(`
          <div style="padding: 16px; min-width: 200px; font-family: system-ui;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
              ...
              <h3 style="margin: 0; font-weight: 600; color: #333;">${order.customerName || 'Cliente'}</h3>
            </div>
            ${order.orderNumber ? `<p style="margin: 6px 0; color: #666; font-size: 14px;">📋 Pedido #${order.orderNumber}</p>` : ''}
            <p style="margin: 6px 0; color: #666; font-size: 14px;">📍 Status: ${order.status || 'Pendente'}</p>
            <p style="margin: 6px 0; color: #999; font-size: 12px;">📍 ${order.latitude.toFixed(6)}, ${order.longitude.toFixed(6)}</p>
            <button 
      ...
      console.error('❌ [MAPBOX] Erro ao criar popup:', error)
    }
  }
  ...
  return (
    <div className={`relative w-full h-full rounded-2xl shadow-lg overflow-hidden ${className}`}>
      ...
      {orders.length > 0 && (
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 text-xs shadow-lg">
          <div className="flex items-center gap-2">
            <span className="text-blue-600">📊</span>
            <span>
              Pedidos: {orders.filter(o => isValidCoordinate(o.latitude, o.longitude)).length}/{orders.length}
            </span>
          </div>
        </div>
      )}
